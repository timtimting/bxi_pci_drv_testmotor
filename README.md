...
该命令会选择当前在线且已使能的电机；离线或未使能的电机会跳过，不影响本次执行。程序将：
...
`-641..641 mm/s`，力矩字段为夹爪力 `-24..24 N`，`Kp` 范围为 `0..5`，`Kd` 范围为 `0..1`。
夹爪固件内部先将夹持力换算为电机侧力矩，再按 `KT` 换算为 q 轴电流；上位机
不应把夹爪的力值当作普通关节的 N·m。示例：
