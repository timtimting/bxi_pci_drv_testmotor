...
static void console_print_help(bool chinese, bool verbose)
{
    ...
    if (chinese) {
        ...
        printf("  maita_pid <bus> <id> [index] [timeout_ms]\n");
        printf("      发送 0x30 读取 PID 参数；不填 index 时读取电流环/速度环/位置环常用 PID。\n");
        printf("  maita_accel <bus> <id> [index] [timeout_ms]\n");
        printf("      发送 0x42 读取加减速度；不填 index 时读取位置/速度规划加速度和减速度。\n");
        printf("  maita_temp <bus> <id> [timeout_ms]\n");
        printf("      发送 0x9A 读取电机温度、电压、抱闸状态和错误码。\n");
        printf("  maita_state <bus> <id> [timeout_ms]\n");
        printf("      发送 0x70 读取电机运行模式，回复 DATA[7]：1 电流环、2 速度环、3 位置环。\n");
    ...
    printf("  maita_pid <bus> <id> [index] [timeout_ms]\n");
    printf("      Send 0x30 to read PID parameters; without index reads common current/speed/position PID.\n");
    printf("  maita_accel <bus> <id> [index] [timeout_ms]\n");
    ...
    printf("  maita_state <bus> <id> [timeout_ms]\n");
    printf("      Send 0x70 to read run mode. Reply DATA[7]: 1 current, 2 speed, 3 position loop.\n");
    printf("  maita_broadcast <bus> <cmd> [d1 d2 d3 d4 d5 d6 d7] [wait_ms]\n");
